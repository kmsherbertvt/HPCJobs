...
function plot_Ωt(t, α, β; ΩMAX=max(maximum(abs.(α)), maximum(abs.(β))),
        tMIN=0.0, tMAX=nothing)
    isnothing(tMAX) && (tMAX = t[end])
    yMAX = ΩMAX / 2π
    plot = Plots.plot(;
        xlabel= "Time (ns)",
        xlims = axislimits(tMIN, tMAX),
...
function plot_ϕt(t, ϕα, ϕβ;
    ...
    #= NOTE: We're plotting -ϕα here,
        since we define gradient signal along descent direction in presentation. =#
...
function plot_pulse_figures(state_label; plot_Δ=false,
        ΩMAX=nothing, tMIN=0.0, tMAX=nothing, kMAX=nothing)
    isnothing(_!.work) && error("Run script before calling `plot` methods.")

    unarchive(state_label)

    t = calculate_timegrid()
    f = calculate_frequencygrid()
    Δ = plot_Δ ? calculate_detunings() : nothing
    ...
    isnothing(ΩMAX) && (ΩMAX = _!.setup.ΩMAX)
    isnothing(tMAX) && (tMAX = t[end])
    isnothing(kMAX) && (kMAX = length(f))

    Ωt = plot_Ωt(t, α, β; ΩMAX=ΩMAX, tMIN=tMIN, tMAX=tMAX)
    ϕt = plot_ϕt(t, ϕα, ϕβ; tMIN=tMIN, tMAX=tMAX)
    ϕf = plot_ϕf(f, ϕ̂α, ϕ̂β; kMAX=kMAX, Δ=Δ)
