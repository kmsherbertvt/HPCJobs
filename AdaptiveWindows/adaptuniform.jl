...
function do_adaptation()
    ...
    # CHECK IF MINIMUM WINDOW DURATION SUPPORTS A NEW WINDOW
    W = length(_!.work.device.Ω̄[1].windows)
    _!.setup.T / (W+1) < _!.setup.ΔsMIN && return false

    # CONSTRUCT TRIAL DEVICE
    candidate = deepcopy(_!.work.device)
    for i in 1:CtrlVQE.ndrives(_!.work.device)
        candidate.Ω̄[i] = JOB.increment_windows(_!.work.device.Ω̄[i], _!.setup.T)
    end
    ...
    Δg = maximum(abs.(g))
    Δg < _!.setup.ΔgMIN && return false     # SIGNAL TERMINATION IF UPDATE HAS NO EFFECT

    # UPDATE ACTUAL DEVICE WITH TRIAL PULSES
    for i in 1:CtrlVQE.ndrives(_!.work.device)
        _!.work.device.Ω̄[i] = candidate.Ω̄[i]
    end
