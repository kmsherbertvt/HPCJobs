...
pulse = CtrlVQE.Signals.WindowedSignal(
    [CtrlVQE.Signals.PolarComplexConstant(zero(Float), zero(Float))],
    [0.0],
)
...
L = length(xi)
# Ω = 1:2:L-n; φ = 2:2:L-n; ν = 1+L-n:L       # Polar with Frequenices
Ω = 1:2:L; φ = 2:2:L; ν = []                # Polar without Frequencies
...
if isnothing(_!.state)
    ...
    for i in 1:CtrlVQE.ndrives(device)
        windows = CtrlVQE.Signals.PolarComplexConstant{Float}[]
        for (k, s) in enumerate(_!.state.s[i])
            push!(windows, CtrlVQE.Signals.PolarComplexConstant(
                _!.state.x[cursor],
                _!.state.x[cursor+1],
            ))
            global cursor += 2
...
function do_optimization()
    ...
    μR = zeros(L); μR[_!.state.Ω] .= +_!.setup.ΩMAX
    μL = zeros(L); μL[_!.state.Ω] .= -_!.setup.ΩMAX
