...
Detuned: Allow pulse frequencies to vary from resonance.
Cartesian: Use complex pulses with Cartesian coordinates but NOT inscribing the bounds.
Smooth: Use smooth bounds for amplitude and drive frequency.

=#

import BFGSTrotterized as JOB
import BFGSTrotterized: _!
...
protopulse = CtrlVQE.ComplexConstant(zero(JOB.Float), zero(JOB.Float))  # CARTESIAN
device = CtrlVQE.Systematic(
    CtrlVQE.TransmonDevice,                                             # DETUNED
    system.n,
    CtrlVQE.FullyTrotterized(protopulse, _!.setup.T, _!.setup.r);
    m = _!.setup.m,
)
...
if isnothing(_!.state)
    ...
    _!.state = JOB.StateVariables(
        x = copy(xi),
        Ω = collect(1:L-n), φ = [], ν = collect(1+L-n:L),               # CARTESIAN DETUNED
...
function bounds(L, ω̄)
    ...
    amplitude_bounds = CtrlVQE.AmplitudeBound(
        _!.setup.ΩMAX, _!.setup.λΩ, _!.setup.σΩ, L, Ω,
        true,                                   # CARTESIAN
    )

    λ = zeros(L); λ[ν] .= _!.setup.λΔ
    σ = zeros(L); σ[ν] .= _!.setup.σΔ
    uB = zeros(L); uB[ν] .= ω̄; uB[ν] .+= _!.setup.ΔMAX
    lB = zeros(L); lB[ν] .= ω̄; lB[ν] .-= _!.setup.ΔMAX
    frequency_bounds = CtrlVQE.SmoothBound(λ, uB, lB, σ)

    return CtrlVQE.CompositeCostFunction(amplitude_bounds, frequency_bounds)
end
