...
Experiments here are each a single optimization,
    so saving the Hessian has no immediately obvious purpose.
Indeed, it would probably be better to use L-BFGS-B (see StrictUniformWindows)
    but we need something concrete to compare against,
    and who knows maybe the Hessian will come in handy someday.
...
module BFGSUniformWindows
    ...
    const SYSTEMPATH = "$(ENV["HOME"])/HPCJobs/BFGSUniformWindows/sys"
    ...
    @with_kw mutable struct SetupVariables
        ...
        ΔsMIN::Float = 3.0 # ns             # SMALLEST WINDOW INTERVAL, AKIN TO BANDWIDTH
            #= NOTE: The idea is to select W ≥ T ÷ ΔsMIN,
                but this is not enforced in the package code. =#
